...
class _LandingScreenState extends State<LandingScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Controlador general para las animaciones de entrada
    _entryAnimationController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 1800), // Más tiempo para la cascada
    );
  ...
  Widget build(BuildContext context) {
    ...
    final colorTextoPrincipal = _isDarkMode ? Colors.white : const Color(0xFF1E293B);
    final colorTextoSecundario = _isDarkMode ? Colors.blueGrey.shade300 : Colors.blueGrey.shade700;
    
    // Tonos tipo cristal
    final colorFondoTarjeta = _isDarkMode ? Colors.white.withOpacity(0.05) : Colors.white.withOpacity(0.6);
    final colorBordeTarjeta = _isDarkMode ? Colors.white.withOpacity(0.1) : Colors.white.withOpacity(0.9);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Padding(
          ...
          child: Row(
            children: [
              const Icon(Icons.record_voice_over, color: Colors.blueAccent, size: 30),
              const SizedBox(width: 10),
              Text('FonoApp', style: TextStyle(fontWeight: FontWeight.w900, color: colorTextoPrincipal, letterSpacing: -0.5)),
            ],
          ),
        ),
        actions: [
          // BOTÓN DE TEMA MÁS ELEGANTE
          GestureDetector(
            onTap: _toggleTheme,
            child: MouseRegion(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: _isDarkMode ? Colors.white.withOpacity(0.1) : Colors.blueGrey.withOpacity(0.1),
                  borderRadius: BorderRadius.circular(30),
                  border: Border.all(color: _isDarkMode ? Colors.white.withOpacity(0.2) : Colors.transparent)
                ),
                child: Row(
                  children: [
                    Text(_isDarkMode ? '🌙' : '☀️', style: const TextStyle(fontSize: 16)),
                    if (!isMobile) ...[
                      const SizedBox(width: 8),
                      Text(_isDarkMode ? 'Oscuro' : 'Claro', style: TextStyle(fontWeight: FontWeight.bold, color: colorTextoPrincipal, fontSize: 13)),
                    ]
                  ],
                ),
              ),
          ...
          if (!isMobile) 
            TextButton(
              ...
              child: Text('Iniciar Sesión', style: TextStyle(fontWeight: FontWeight.bold, color: colorTextoSecundario, fontSize: 16)),
            ),
          const SizedBox(width: 10),
          Padding(
            padding: EdgeInsets.only(right: isMobile ? 10 : 30.0),
            child: ElevatedButton(
              ...
              child: const Text('Registrarse', style: TextStyle(fontWeight: FontWeight.w900)),
            ),
          ),
        ],
      ),
      body: Stack(
        children: [
          ...
          // CAPA 2: CONTENIDO SCROLLABLE (Ocupa toda la pantalla para arreglar el bug)
          Positioned.fill(
            child: SingleChildScrollView(
              physics: const BouncingScrollPhysics(parent: AlwaysScrollableScrollPhysics()), // Scroll fluido en cualquier parte
              child: SafeArea(
                child: Center(
                  child: ConstrainedBox(
                    constraints: const BoxConstraints(maxWidth: 1200), // Evita que se estire demasiado en monitores gigantes
                    child: Padding(
                        ...
                        children: [
                          ...
                          _FadeSlide(
                            ...
                            child: Container(
                              ...
                              child: const Text('🚀 El SAAC más intuitivo de la web', style: TextStyle(fontWeight: FontWeight.w900, color: Colors.blueAccent, letterSpacing: 0.5)),
                            ),
                          ),
                          ...
                          _FadeSlide(
                            ...
                            child: ShaderMask(
                              shaderCallback: (bounds) => LinearGradient(
                                colors: _isDarkMode ? [Colors.blue.shade300, Colors.purple.shade300] : [Colors.blue.shade700, Colors.purple.shade700],
                                begin: Alignment.topLeft,
                                end: Alignment.bottomRight,
                              ).createShader(bounds),
                              child: Text(
                          ...
                          _FadeSlide(
                            ...
                            child: Text('Elige tu nivel de acceso', style: TextStyle(fontSize: isMobile ? 32 : 45, fontWeight: FontWeight.w900, color: colorTextoPrincipal, letterSpacing: -1)),
                          ),
                          ...
                          const SizedBox(height: 60),
                          Text('© 2026 FonoApp. Desarrollado con propósito y dedicación.', style: TextStyle(color: colorTextoSecundario, fontSize: 14)),
                        ],
                      ),
                    ),
                  ),
...
// Widget para que los elementos entren flotando y desvaneciéndose (Staggered Animation)
  ...
  Widget build(BuildContext context) {
    return MouseRegion(
      ...
      child: AnimatedContainer(
        ...
        transform: Matrix4.translationValues(0, _isHovered ? -10 : 0, 0), // Flota hacia arriba
        width: 280,
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: const Row(
            ...
            children: [
              Text('CREAR CUENTA AHORA', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
              SizedBox(width: 15),
